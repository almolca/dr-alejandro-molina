...
(async () => {
  ...
  fs.writeFileSync('qa/r4-2/results.json',JSON.stringify(report,null,2));
  console.log(JSON.stringify({pages:results.length,failures:results.filter(r=>r.status!==200||r.overflow||r.h1!==1||r.gated||r.errors.length||!r.links||r.logo.some(l=>!l.loaded)||r.photoFrames.some(f=>!f.insideViewport||f.width<=0||f.height<=0||!f.emptyText)||(['/', '/about'].includes(r.route)&&!r.approvedRecognition)),interactions,accessibility,reducedMotion},null,2));
  await browser.close();
