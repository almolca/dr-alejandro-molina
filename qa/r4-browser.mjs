...
const routes = ['/', '/about', '/male-aesthetics', flagship, '/mens-health', '/sexual-medicine', '/penile-surgery'];
...
(async () => {
  ...
  for(const route of routes) for(const width of widths) {
    ...
    const metrics=await page.evaluate(() => ({
      ...
      gated:/EXACT OFFICIAL|PRP|P-Shot/.test(document.body.innerText),
  ...
  const accessibility=[];
  if(process.env.R4_AXE_PATH) for(const route of routes.slice(0,4)) for(const width of [390,1440]) {
    await page.setViewportSize({width,height:1000});await page.goto(base+route);await page.waitForTimeout(600);
  ...
  fs.writeFileSync('qa/r4-results.json',JSON.stringify(report,null,2));
  console.log(JSON.stringify({pages:results.length,failures:results.filter(r=>r.status!==200||r.overflow||r.h1!==1||r.gated||r.errors.length||!r.links||r.logo.some(l=>!l.loaded)),interactions,accessibility,reducedMotion},null,2));
  await browser.close();
