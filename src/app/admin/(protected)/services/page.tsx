...
const columns: Column<ServicePerformanceRow>[] = [
  { key: "service", header: "Service", render: (r) => serviceInterestLabel(r.service_interest) },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl text-stone-900">Service Interest</h1>
          <p className="mt-1 text-sm text-stone-500">{label}</p>
        </div>
        <TimeRangePicker basePath="/admin/services" currentRange={rangeParam} />
      </div>

      <DataTable columns={columns} rows={rows} rowKey={(r) => r.service_interest} />
    </div>
  );
}
